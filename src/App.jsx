...
function Home({xp,done,quizLog,setPage}){
  ...
  return(<div style={{overflowX:"hidden",width:"100%"}}>
    ...
    <section className="page-section"><div className="content-wrap">
      ...
      <div className="module-grid">{[MODULES[0],MODULES[1],MODULES.find(m=>m.level==="Advanced"),MODULES.find(m=>m.level==="Intermediate")].map(m=><ModCard key={m.id} mod={m} done={done} onClick={()=>setPage("courses")}/>)}</div>
    </div></section>
