...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero-title { font-family:'Syne',sans-serif; font-size:clamp(1.8rem,4vw,3.8rem); font-weight:800; line-height:1.15; margin-bottom:16px; letter-spacing:-0.02em; width:100%; overflow-wrap:break-word; word-break:break-word; }
  .hero-title .gradient-text { background:linear-gradient(90deg,#22d3ee 0%,#60a5fa 50%,#a78bfa 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; display:inline; }
  ...
  .stat-val { font-family:'Syne',sans-serif; font-size:1.9rem; font-weight:800; }
  .stat-lbl { color:rgba(255,255,255,0.35); font-size:0.75rem; margin-top:3px; }
  /* Sections */
  .section-title { font-family:'Syne',sans-serif; font-size:1.5rem; font-weight:800; margin-bottom:4px; }
  ...
  /* CTA */
  .cta-title { font-family:'Syne',sans-serif; font-size:2rem; font-weight:800; margin-bottom:10px; }
  /* Lessons */
  ...
  .btn-secondary { padding:12px 22px; border-radius:12px; background:rgba(255,255,255,0.04); color:white; border:1px solid rgba(255,255,255,0.1); cursor:pointer; font-weight:600; font-size:0.9rem; font-family:'DM Sans',sans-serif; }
  ...
  @media (max-width: 768px) {
    ...
    .hero-section { min-height:0 !important; height:auto !important; display:block !important; padding:80px 16px 36px !important; overflow:hidden !important; }
    .hero-content { text-align:center; width:100% !important; max-width:100% !important; padding:0; }
    .hero-title { font-size:clamp(1.6rem,7vw,2.2rem) !important; line-height:1.2 !important; margin-bottom:12px; letter-spacing:-0.01em !important; }
    .hero-p { font-size:0.88rem; margin-bottom:18px; padding:0 4px; }
    .hero-badge { font-size:0.7rem; padding:4px 10px; margin-bottom:14px; }
    .hero-btns { flex-direction:column; align-items:stretch; gap:8px; margin-bottom:18px; }
    .hero-btns .btn-primary, .hero-btns .btn-secondary { width:100%; text-align:center; padding:13px; }
    .hero-tags { gap:5px; padding:0 2px; }
  ...
  @media (max-width: 430px) {
    ...
    .hero-tags { gap:4px; }
    .hero-tag { font-size:0.68rem; padding:3px 8px; }
  }
  ...
  @media (max-height: 500px) and (orientation: landscape) {
    ...
    .hero-title { font-size:clamp(1.2rem,3.5vw,1.8rem) !important; margin-bottom:8px; }
  ...
  @media (min-width: 769px) and (max-width: 1024px) {
    ...
    .hero-title { font-size:clamp(1.8rem,3.5vw,2.8rem) !important; }
  }
`;
    ...
    <nav className="nav-bar">
      <div className="nav-inner">
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          ...
          {!installed && installPrompt && (
            <button onClick={install} title="Install app" style={{display:"flex",alignItems:"center",gap:5,padding:"4px 10px",borderRadius:20,background:"linear-gradient(135deg,rgba(34,211,238,0.15),rgba(59,130,246,0.15))",border:"1px solid rgba(34,211,238,0.3)",color:"#22d3ee",fontSize:"0.75rem",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",flexShrink:0}}>
              📲 Install
            </button>
          )}
          {installed && (
            <span style={{fontSize:"0.72rem",color:"rgba(34,211,238,0.5)",flexShrink:0}}>✓ Installed</span>
          )}
      ...
      {menuOpen&&(
        <div className="nav-mobile-menu">
          {navItems.map(([id,icon,lbl])=>(
            <button key={id} onClick={()=>{setPage(id);setMenuOpen(false);}} className={`nav-mobile-btn${page===id?" active":""}`}>
              <span style={{fontSize:"1rem"}}>{icon}</span>{lbl}
            </button>
          ))}
          {!installed && installPrompt && (
            <button onClick={()=>{install();setMenuOpen(false);}} className="nav-mobile-btn" style={{color:"#22d3ee",marginTop:4,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:12}}>
              <span style={{fontSize:"1rem"}}>📲</span> Install App
            </button>
          )}
          {installed && (
            <div style={{padding:"10px 12px",fontSize:"0.78rem",color:"rgba(34,211,238,0.5)",borderTop:"1px solid rgba(255,255,255,0.06)",marginTop:4}}>
              ✓ App installed
            </div>
          )}
        </div>
...
function ModCard({mod,done,onClick}){
  ...
  return(
    <div onClick={onClick} className="mod-card" style={{border:`1px solid ${mod.color}22`,background:`linear-gradient(135deg,${mod.color}10,${mod.color}05)`}}>
  ...
  return(
    <div style={{overflowX:"hidden",width:"100%"}}>
      ...
      <section className="hero-section">
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 70% 50% at 30% 20%,rgba(34,211,238,0.06) 0%,transparent 60%),radial-gradient(ellipse 60% 50% at 70% 80%,rgba(59,130,246,0.06) 0%,transparent 60%)",overflow:"hidden"}}/>
        <div className="hero-content">
          <div className="hero-badge">⭐ AI-Powered Biomedical Engineering Platform</div>
          <h1 className="hero-title">
            <span>Master </span>
            <span className="gradient-text">Biomedical</span>
      ...
      <div className="stats-bar">
        <div className="stats-grid">
          {[[xp.toLocaleString(),"XP Earned","#facc15"],[`${done.length}/${total}`,"Lessons Done","#22d3ee"],[quizLog.length,"Quizzes Taken","#a78bfa"],[`${pct}%`,"Progress","#34d399"]].map(([v,l,c])=>(
            <div key={l}>
              <div className="stat-val" style={{background:`linear-gradient(135deg,${c},rgba(255,255,255,0.9))`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{v}</div>
...
function Courses({done,quizLog,completeLesson,logQuiz}){
  ...
  return(
    <div style={{maxWidth:1000,margin:"0 auto",padding:"36px 20px"}}>
      ...
      <p className="section-sub" style={{marginBottom:24}}>Select a module to start learning and earning XP</p>
      <div className="module-grid">
  ...
  return(
    <div style={{maxWidth:760,margin:"0 auto",padding:"28px 16px"}}>
          ...
          return(
            <div key={l.id} className="lesson-row" style={{border:`1px solid ${isDone?mod.color+"33":"rgba(255,255,255,0.06)"}`}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div className="lesson-title" style={{color:isDone?"rgba(255,255,255,0.88)":"rgba(255,255,255,0.65)"}}>{l.title}</div>
              ...
              {isDone
                ...
                :<button onClick={()=>completeLesson(l.id)} style={{padding:"4px 10px",borderRadius:7,background:`${mod.color}12`,border:`1px solid ${mod.color}2e`,color:mod.color,fontSize:"0.7rem",cursor:"pointer",fontWeight:600,flexShrink:0,whiteSpace:"nowrap",fontFamily:"'DM Sans',sans-serif"}}>
                  +10 XP
                </button>
              }
            </div>
      ...
      <div style={{borderRadius:14,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",padding:16,display:"flex",justifyContent:"space-between",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        ...
        <button onClick={onStartQuiz} style={{padding:"9px 16px",borderRadius:10,background:mod.grad,color:"white",border:"none",cursor:"pointer",fontWeight:700,fontSize:"0.82rem",whiteSpace:"nowrap",fontFamily:"'DM Sans',sans-serif"}}>
    ...
    return(
      <div style={{maxWidth:500,margin:"0 auto",padding:"48px 20px",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={onDone} style={{padding:"11px 20px",borderRadius:10,background:mod.grad,color:"white",border:"none",cursor:"pointer",fontWeight:700,fontSize:"0.875rem",fontFamily:"'DM Sans',sans-serif"}}>Back to Module</button>
          <button onClick={()=>setQs({idx:0,answers:[],score:0,selected:null,done:false})} style={{padding:"11px 20px",borderRadius:10,background:"rgba(255,255,255,0.05)",color:"white",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",fontWeight:700,fontSize:"0.875rem",fontFamily:"'DM Sans',sans-serif"}}>Retake</button>
        </div>
      </div>
    );
  }
    ...
    <div style={{maxWidth:600,margin:"0 auto",padding:"28px 16px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {q.o.map((opt,i)=>{
          ...
          return(
            <button key={i} onClick={()=>pick(i)} disabled={qs.selected!==null} className="opt-btn"
              style={{background:bg,border:`1px solid ${bdr}`,color:clr,cursor:qs.selected!==null?"default":"pointer"}}>
  ...
  return(
    <div className="tutor-wrap">
        ...
        {msgs.length===0&&(
          <div style={{textAlign:"center",paddingTop:40}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center",padding:"0 10px"}}>
              {prompts.map(p=>(
                <button key={p} onClick={()=>setInput(p)} style={{padding:"7px 12px",borderRadius:20,background:"rgba(34,211,238,0.07)",border:"1px solid rgba(34,211,238,0.18)",color:"#22d3ee",fontSize:"0.76rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{p}</button>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:10,flexShrink:0}}>
        {msgs.length>0&&(
          <button onClick={()=>{setMsgs([]);saveChat([]);}} style={{background:"none",border:"none",color:"rgba(255,255,255,0.2)",fontSize:"0.7rem",cursor:"pointer",marginBottom:7,display:"block",fontFamily:"'DM Sans',sans-serif"}}>
            🗑 Clear history
          </button>
        )}
        <div style={{display:"flex",gap:8,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:12,padding:"8px 12px",alignItems:"flex-end"}}>
          <textarea value={input} onChange={e=>setInput(e.target.value)}
            ...
            placeholder="Ask about MRI, BCIs, biosensors…" rows={2}
...
function MindMap(){
  ...
  return(
    <div style={{maxWidth:1100,margin:"0 auto",padding:"36px 16px"}}>
      ...
      <p className="section-sub" style={{marginBottom:24}}>All biomedical engineering topics at a glance</p>
      <div className="module-grid">
        {topics.map(t=>(
