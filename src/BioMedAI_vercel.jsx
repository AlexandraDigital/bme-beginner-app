...
export default function App(){
  ...
  return(
    <>
      ...
      `}</style>
      <div style={{background:"#060b18",minHeight:"100vh",color:"white",fontFamily:"'DM Sans',sans-serif"}}>
...
function Nav({page,setPage,xp}){
  return(
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:50,background:"rgba(6,11,24,0.92)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 20px",height:60,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <button onClick={()=>setPage("home")} style={{display:"flex",alignItems:"center",gap:10,background:"none",border:"none",cursor:"pointer",padding:0}}>
          <div style={{width:34,height:34,borderRadius:10,background:"linear-gradient(135deg,#22d3ee,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 20px rgba(34,211,238,0.22)"}}>
        ...
        <div style={{display:"flex",gap:2}}>
          {[["home","🧠","Home"],["courses","📚","Courses"],["mindmap","🗺️","Mind Map"],["tutor","🤖","AI Tutor"]].map(([id,icon,lbl])=>(
            <button key={id} onClick={()=>setPage(id)} className="nav-btn" style={{display:"flex",alignItems:"center",gap:6,padding:"7px 13px",borderRadius:8,background:page===id?"rgba(34,211,238,0.1)":"transparent",color:page===id?"#22d3ee":"rgba(255,255,255,0.4)",border:"none",cursor:"pointer",fontSize:"0.82rem",fontWeight:500,transition:"all 0.15s"}}>
              {icon} {lbl}
            </button>
          ))}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:20,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)"}}>
          ...
          <span style={{color:"#facc15",fontWeight:700,fontSize:"0.85rem"}}>{xp.toLocaleString()} XP</span>
        </div>
      </div>
    </nav>
  );
...
function ModCard({mod,done,onClick}){
  ...
  return(
    <div onClick={onClick} className="mod-card" style={{borderRadius:16,border:`1px solid ${mod.color}22`,padding:22,cursor:"pointer",background:`linear-gradient(135deg,${mod.color}10,${mod.color}05)`,transition:"transform 0.2s"}}>
      ...
      <h3 style={{fontWeight:700,fontSize:"0.97rem",marginBottom:10,fontFamily:"Syne,sans-serif"}}>{mod.title}</h3>
...
function Home({xp,done,quizLog,setPage}){
  ...
  return(
    <div>
      {/* Hero */}
      <section style={{minHeight:"88vh",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",padding:"40px 20px"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:780,textAlign:"center"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"5px 14px",borderRadius:20,background:"rgba(34,211,238,0.07)",border:"1px solid rgba(34,211,238,0.18)",color:"#22d3ee",fontSize:"0.78rem",marginBottom:22,fontWeight:500}}>
            ⭐ AI-Powered Biomedical Engineering Platform
          </div>
          <h1 style={{fontFamily:"Syne,sans-serif",fontSize:"clamp(2.6rem,6.5vw,4.8rem)",fontWeight:800,lineHeight:1.1,marginBottom:18,letterSpacing:"-0.025em"}}>
            <span>Master </span>
          ...
          <p style={{color:"rgba(255,255,255,0.45)",fontSize:"1.05rem",maxWidth:520,margin:"0 auto 30px",lineHeight:1.65}}>
            Explore MRI physics, tissue engineering, neural BCIs, and more — guided by an AI tutor answering your questions in real time.
          </p>
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap",marginBottom:36}}>
            <button onClick={()=>setPage("courses")} style={{padding:"13px 26px",borderRadius:12,background:"linear-gradient(135deg,#22d3ee,#3b82f6)",color:"white",border:"none",cursor:"pointer",fontWeight:600,fontSize:"0.95rem",boxShadow:"0 0 28px rgba(34,211,238,0.2)"}}>Explore Courses →</button>
            <button onClick={()=>setPage("tutor")} style={{padding:"13px 26px",borderRadius:12,background:"rgba(255,255,255,0.04)",color:"white",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",fontWeight:600,fontSize:"0.95rem"}}>🤖 Talk to AI Tutor</button>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center"}}>
            {["🩻 Medical Imaging","🦴 Biomechanics","🧬 Tissue Engineering","🧠 Neural Engineering","📡 Biosensors","💻 Bioinformatics"].map(t=>(
              <span key={t} style={{padding:"5px 12px",borderRadius:20,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.45)",fontSize:"0.78rem"}}>{t}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)",background:"rgba(255,255,255,0.015)"}}>
        <div style={{maxWidth:900,margin:"0 auto",padding:"28px 20px",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,textAlign:"center"}}>
          {[[xp.toLocaleString(),"XP Earned","#facc15"],[`${done.length}/${total}`,"Lessons Done","#22d3ee"],[quizLog.length,"Quizzes Taken","#a78bfa"],[`${pct}%`,"Progress","#34d399"]].map(([v,l,c])=>(
            <div key={l}>
              ...
              <div style={{color:"rgba(255,255,255,0.35)",fontSize:"0.78rem",marginTop:3}}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section style={{maxWidth:1000,margin:"0 auto",padding:"56px 20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:28}}>
          <div>
            <h2 style={{fontFamily:"Syne,sans-serif",fontSize:"1.7rem",fontWeight:800,marginBottom:5}}>Featured Modules</h2>
            <p style={{color:"rgba(255,255,255,0.38)",fontSize:"0.85rem"}}>Start your journey with these popular courses</p>
          </div>
      ...
      {/* How it works */}
      <section style={{borderTop:"1px solid rgba(255,255,255,0.05)",padding:"56px 20px",background:"rgba(255,255,255,0.01)"}}>
        <div style={{maxWidth:920,margin:"0 auto"}}>
          <h2 style={{fontFamily:"Syne,sans-serif",fontSize:"1.7rem",fontWeight:800,textAlign:"center",marginBottom:6}}>How It Works</h2>
          <p style={{color:"rgba(255,255,255,0.38)",textAlign:"center",marginBottom:36,fontSize:"0.85rem"}}>Everything you need to master biomedical engineering</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:14}}>
            {[{e:"🤖",t:"AI Tutor",d:"Ask anything, get expert explanations instantly.",g:"#22d3ee,#3b82f6"},{e:"📚",t:"Structured Courses",d:"3 tiers: Beginner, Intermediate & Advanced.",g:"#34d399,#0d9488"},{e:"🧪",t:"Interactive Quizzes",d:"125+ questions with detailed answer breakdowns.",g:"#a78bfa,#7c3aed"},{e:"⚡",t:"Track Progress",d:"Earn XP, complete lessons, and watch your stats grow.",g:"#fb923c,#ef4444"}].map(f=>(
              <div key={f.t} style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:18}}>
                <div style={{width:42,height:42,borderRadius:11,background:`linear-gradient(135deg,${f.g})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.2rem",marginBottom:10}}>{f.e}</div>
                <h3 style={{fontWeight:700,marginBottom:5,fontSize:"0.9rem",fontFamily:"Syne,sans-serif"}}>{f.t}</h3>
                <p style={{color:"rgba(255,255,255,0.38)",fontSize:"0.8rem",lineHeight:1.5}}>{f.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{padding:"72px 20px",textAlign:"center"}}>
        <h2 style={{fontFamily:"Syne,sans-serif",fontSize:"2rem",fontWeight:800,marginBottom:10}}>The Future of Medicine Needs Engineers</h2>
        <p style={{color:"rgba(255,255,255,0.4)",margin:"0 auto 26px",maxWidth:460,fontSize:"0.9rem"}}>Biomedical engineering bridges medicine and technology. Start building your expertise today.</p>
        <button onClick={()=>setPage("courses")} style={{padding:"13px 30px",borderRadius:12,background:"linear-gradient(135deg,#22d3ee,#3b82f6)",color:"white",border:"none",cursor:"pointer",fontWeight:700,fontSize:"0.95rem"}}>Get Started Free →</button>
      </section>
    </div>
  );
}
...
function Courses({done,quizLog,completeLesson,logQuiz}){
  ...
  return(
    <div style={{maxWidth:1000,margin:"0 auto",padding:"40px 20px"}}>
      <h1 style={{fontFamily:"Syne,sans-serif",fontSize:"1.9rem",fontWeight:800,marginBottom:6}}>All Courses</h1>
      <p style={{color:"rgba(255,255,255,0.38)",marginBottom:28,fontSize:"0.85rem"}}>Select a module to start learning and earning XP</p>
...
function ModDetail({mod,done,completeLesson,quizLog,onBack,onStartQuiz}){
  ...
  return(
    <div style={{maxWidth:760,margin:"0 auto",padding:"36px 20px"}}>
      <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:7,background:"none",border:"none",color:"rgba(255,255,255,0.4)",cursor:"pointer",marginBottom:22,fontSize:"0.82rem"}}>
        ← Back to Courses
      </button>
      <div style={{borderRadius:18,border:`1px solid ${mod.color}2a`,padding:24,background:`linear-gradient(135deg,${mod.color}0d,${mod.color}06)`,marginBottom:20}}>
        <div style={{fontSize:"2.2rem",marginBottom:10}}>{mod.emoji}</div>
        <h1 style={{fontFamily:"Syne,sans-serif",fontSize:"1.5rem",fontWeight:800,marginBottom:6}}>{mod.title}</h1>
        <div style={{display:"flex",gap:14,color:"rgba(255,255,255,0.38)",fontSize:"0.8rem",marginBottom:14}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",fontSize:"0.78rem",color:"rgba(255,255,255,0.35)"}}>
          <span>{n}/{mod.lessons.length} completed</span><span>{pct}%</span>
        </div>
      </div>

      <h2 style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:"1rem",marginBottom:12}}>Lessons</h2>
      <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:24}}>
        {mod.lessons.map((l,i)=>{
          const isDone=done.includes(l.id);
          return(
            <div key={l.id} className="lesson-row" style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderRadius:11,background:"rgba(255,255,255,0.02)",border:`1px solid ${isDone?mod.color+"33":"rgba(255,255,255,0.06)"}`,transition:"all 0.18s"}}>
              <div style={{width:26,height:26,borderRadius:"50%",background:isDone?`${mod.color}1a`:"rgba(255,255,255,0.04)",border:`1.5px solid ${isDone?mod.color:"rgba(255,255,255,0.12)"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.72rem",color:isDone?mod.color:"rgba(255,255,255,0.28)",flexShrink:0,fontFamily:"Syne,sans-serif",fontWeight:800}}>
                {isDone?"✓":i+1}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:"0.875rem",fontWeight:500,color:isDone?"rgba(255,255,255,0.88)":"rgba(255,255,255,0.65)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{l.title}</div>
                <div style={{fontSize:"0.72rem",color:"rgba(255,255,255,0.28)",marginTop:1}}>{l.mins} min</div>
              </div>
              {isDone
                ?<span style={{fontSize:"0.72rem",color:mod.color,fontWeight:600,flexShrink:0}}>✓ Done</span>
                :<button onClick={()=>completeLesson(l.id)} style={{padding:"4px 11px",borderRadius:7,background:`${mod.color}12`,border:`1px solid ${mod.color}2e`,color:mod.color,fontSize:"0.72rem",cursor:"pointer",fontWeight:600,flexShrink:0,whiteSpace:"nowrap"}}>
                  Mark done +10 XP
                </button>
              }
            </div>
      ...
      <div style={{borderRadius:14,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",padding:18,display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,flexWrap:"wrap"}}>
        <div>
          <h3 style={{fontWeight:700,marginBottom:3,fontSize:"0.95rem"}}>📝 Module Quiz</h3>
          <p style={{color:"rgba(255,255,255,0.38)",fontSize:"0.78rem"}}>{mod.quiz.length} questions • Up to {mod.quiz.length*5+25} XP</p>
          {best!==null&&<p style={{color:mod.color,fontSize:"0.78rem",marginTop:3}}>Best score: {best}%</p>}
          {quizLog.length>0&&<p style={{color:"rgba(255,255,255,0.25)",fontSize:"0.73rem"}}>Taken {quizLog.length}×</p>}
        </div>
        <button onClick={onStartQuiz} style={{padding:"9px 18px",borderRadius:10,background:mod.grad,color:"white",border:"none",cursor:"pointer",fontWeight:700,fontSize:"0.83rem",whiteSpace:"nowrap"}}>
...
function Quiz({mod,qs,setQs,logQuiz,onBack,onDone}){
  ...
  if(qs.done){
    ...
    return(
      <div style={{maxWidth:540,margin:"0 auto",padding:"60px 20px",textAlign:"center"}}>
        <div style={{fontSize:"3.5rem",marginBottom:14}}>{pct>=80?"🎉":pct>=60?"👍":"📚"}</div>
        <h2 style={{fontFamily:"Syne,sans-serif",fontSize:"1.9rem",fontWeight:800,marginBottom:8}}>Quiz Complete!</h2>
        <div style={{fontFamily:"Syne,sans-serif",fontSize:"3.2rem",fontWeight:800,background:`linear-gradient(135deg,${mod.color},white)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:6}}>{pct}%</div>
        <p style={{color:"rgba(255,255,255,0.45)",marginBottom:8,fontSize:"0.9rem"}}>{qs.score}/{mod.quiz.length} correct</p>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"7px 16px",borderRadius:20,background:"rgba(250,204,21,0.1)",border:"1px solid rgba(250,204,21,0.2)",color:"#facc15",marginBottom:30,fontSize:"0.875rem",fontWeight:600}}>
          ⚡ +{qs.xpEarned} XP earned!
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <button onClick={onDone} style={{padding:"11px 22px",borderRadius:10,background:mod.grad,color:"white",border:"none",cursor:"pointer",fontWeight:700,fontSize:"0.875rem"}}>Back to Module</button>
          <button onClick={()=>setQs({idx:0,answers:[],score:0,selected:null,done:false})} style={{padding:"11px 22px",borderRadius:10,background:"rgba(255,255,255,0.05)",color:"white",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",fontWeight:700,fontSize:"0.875rem"}}>Retake</button>
        </div>
      </div>
    );
  }
  ...
  return(
    <div style={{maxWidth:620,margin:"0 auto",padding:"36px 20px"}}>
      <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:7,background:"none",border:"none",color:"rgba(255,255,255,0.4)",cursor:"pointer",marginBottom:20,fontSize:"0.82rem"}}>← Back</button>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <span style={{color:"rgba(255,255,255,0.35)",fontSize:"0.78rem"}}>Question {qs.idx+1} of {mod.quiz.length}</span>
      ...
      <div style={{borderRadius:14,background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",padding:24,marginBottom:14}}>
        <p style={{fontSize:"1rem",fontWeight:600,lineHeight:1.55}}>{q.q}</p>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:9}}>
        {q.o.map((opt,i)=>{
          ...
          return(
            <button key={i} onClick={()=>pick(i)} disabled={qs.selected!==null} className="opt-btn" style={{padding:"13px 16px",borderRadius:11,background:bg,border:`1px solid ${bdr}`,color:clr,textAlign:"left",cursor:qs.selected!==null?"default":"pointer",fontSize:"0.875rem",fontWeight:500,transition:"all 0.2s",lineHeight:1.4,width:"100%"}}>
              <span style={{opacity:0.45,marginRight:10,fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:"0.78rem"}}>{String.fromCharCode(65+i)}.</span>
              {opt}
            </button>
          );
        })}
...
function Tutor({chat,saveChat}){
  ...
  const prompts=["Explain MRI physics simply","How do cochlear implants work?","What is CRISPR used for in BME?","Difference between CT and PET scans?"];
  return(
    <div style={{maxWidth:760,margin:"0 auto",padding:"32px 20px",height:"calc(100vh - 92px)",display:"flex",flexDirection:"column"}}>
      <div style={{marginBottom:16}}>
        <h1 style={{fontFamily:"Syne,sans-serif",fontSize:"1.7rem",fontWeight:800,marginBottom:3}}>🤖 AI Tutor</h1>
        <p style={{color:"rgba(255,255,255,0.35)",fontSize:"0.82rem"}}>Ask anything about biomedical engineering • Conversation saved automatically</p>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:14,paddingBottom:12}}>
        {msgs.length===0&&(
          <div style={{textAlign:"center",paddingTop:56}}>
            ...
            <h3 style={{fontFamily:"Syne,sans-serif",fontWeight:700,marginBottom:7,fontSize:"1.1rem"}}>Ask me anything about BME</h3>
            <p style={{color:"rgba(255,255,255,0.3)",fontSize:"0.83rem",marginBottom:22}}>Your personal Biomedical Engineering tutor</p>
            <div style={{display:"flex",flexWrap:"wrap",gap:7,justifyContent:"center"}}>
              {prompts.map(p=>(
                <button key={p} onClick={()=>setInput(p)} style={{padding:"7px 14px",borderRadius:20,background:"rgba(34,211,238,0.07)",border:"1px solid rgba(34,211,238,0.18)",color:"#22d3ee",fontSize:"0.78rem",cursor:"pointer"}}>{p}</button>
              ))}
            </div>
          </div>
        )}
        {msgs.map((m,i)=>(
          <div key={i} className="msg" style={{display:"flex",gap:10,justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            {m.role==="assistant"&&(
              <div style={{width:30,height:30,borderRadius:9,background:"linear-gradient(135deg,#22d3ee,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.8rem",flexShrink:0,marginTop:2}}>🤖</div>
            )}
            <div style={{maxWidth:"80%",padding:"11px 14px",fontSize:"0.875rem",lineHeight:1.6,borderRadius:m.role==="user"?"14px 14px 3px 14px":"14px 14px 14px 3px",background:m.role==="user"?"linear-gradient(135deg,rgba(34,211,238,0.12),rgba(59,130,246,0.12))":"rgba(255,255,255,0.035)",border:`1px solid ${m.role==="user"?"rgba(34,211,238,0.18)":"rgba(255,255,255,0.06)"}`,color:"rgba(255,255,255,0.88)",whiteSpace:"pre-wrap",wordBreak:"break-word"}}>
              {m.content}
            </div>
          </div>
        ))}
        {busy&&(
          <div style={{display:"flex",gap:10}}>
            <div style={{width:30,height:30,borderRadius:9,background:"linear-gradient(135deg,#22d3ee,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.8rem",flexShrink:0}}>🤖</div>
            <div style={{padding:"11px 16px",borderRadius:"14px 14px 14px 3px",background:"rgba(255,255,255,0.035)",border:"1px solid rgba(255,255,255,0.06)",display:"flex",gap:5,alignItems:"center"}}>
              {[0,1,2].map(j=><div key={j} style={{width:6,height:6,borderRadius:"50%",background:"#22d3ee",animation:`bounce 1s ${j*0.18}s infinite`}}/>)}
            </div>
          </div>
        )}
        <div ref={bottomRef}/>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:14}}>
        {msgs.length>0&&(
          <button onClick={()=>{setMsgs([]);saveChat([]);}} style={{background:"none",border:"none",color:"rgba(255,255,255,0.2)",fontSize:"0.72rem",cursor:"pointer",marginBottom:9,display:"block"}}>
            🗑 Clear history
          </button>
        )}
        <div style={{display:"flex",gap:9,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:13,padding:"9px 13px"}}>
          <textarea value={input} onChange={e=>setInput(e.target.value)}
            onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}}
            placeholder="Ask about MRI, BCIs, tissue engineering, biosensors…" rows={2}
            style={{flex:1,background:"none",border:"none",color:"white",resize:"none",fontSize:"0.875rem",outline:"none",lineHeight:1.5,fontFamily:"'DM Sans',sans-serif"}}
          />
          <button onClick={send} disabled={busy||!input.trim()} style={{width:38,height:38,borderRadius:10,alignSelf:"flex-end",flexShrink:0,background:input.trim()&&!busy?"linear-gradient(135deg,#22d3ee,#3b82f6)":"rgba(255,255,255,0.04)",border:"none",cursor:input.trim()&&!busy?"pointer":"default",color:"white",fontSize:"1rem",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s"}}>→</button>
        </div>
        <div style={{textAlign:"center",marginTop:6,color:"rgba(255,255,255,0.18)",fontSize:"0.7rem"}}>Enter to send • Shift+Enter for new line</div>
      </div>
    </div>
  );
}
...
function MindMap(){
  ...
  return(
    <div style={{maxWidth:1100,margin:"0 auto",padding:"40px 20px"}}>
      <h1 style={{fontFamily:"Syne,sans-serif",fontSize:"1.9rem",fontWeight:800,marginBottom:5}}>🗺️ Mind Map</h1>
      <p style={{color:"rgba(255,255,255,0.38)",marginBottom:28,fontSize:"0.85rem"}}>All biomedical engineering topics at a glance</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}}>
        {topics.map(t=>(
          <div key={t.cat} style={{borderRadius:14,border:`1px solid ${t.color}20`,background:`${t.color}06`,padding:18}}>
            <h3 style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:"0.9rem",color:t.color,marginBottom:12}}>{t.cat}</h3>
            <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
              {t.items.map(item=>(
                <span key={item} style={{padding:"3px 9px",borderRadius:20,background:`${t.color}10`,border:`1px solid ${t.color}25`,color:"rgba(255,255,255,0.65)",fontSize:"0.75rem"}}>{item}</span>
              ))}
            </div>
          </div>
        ))}
