...
function Home({xp,done,quizLog,setPage}){
  ...
  return(
    <div>
      {/* Hero */}
      <section style={{minHeight:isMobile?"auto":"88vh",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",padding:isMobile?"60px 20px 40px":"40px 20px"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:780,width:"100%",textAlign:"center"}}>
          ...
          <p style={{color:"rgba(255,255,255,0.45)",fontSize:isMobile?"0.9rem":"1.05rem",maxWidth:520,margin:"0 auto 26px",lineHeight:1.65}}>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)",background:"rgba(255,255,255,0.015)"}}>
        <div style={{maxWidth:900,margin:"0 auto",padding:"24px 20px",display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:16,textAlign:"center"}}>
          {[[xp.toLocaleString(),"XP Earned","#facc15"],[`${done.length}/${total}`,"Lessons Done","#22d3ee"],[quizLog.length,"Quizzes Taken","#a78bfa"],[`${pct}%`,"Progress","#34d399"]].map(([v,l,c])=>(
            <div key={l}>
              <div style={{fontFamily:"Syne,sans-serif",fontSize:isMobile?"1.5rem":"1.9rem",fontWeight:800,background:`linear-gradient(135deg,${c},rgba(255,255,255,0.9))`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{v}</div>
      ...
      <section style={{padding:"60px 20px",textAlign:"center"}}>
        <h2 style={{fontFamily:"Syne,sans-serif",fontSize:isMobile?"1.5rem":"2rem",fontWeight:800,marginBottom:10}}>The Future of Medicine Needs Engineers</h2>
          ...
          return(
            <div key={l.id} className="lesson-row" style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderRadius:11,background:"rgba(255,255,255,0.02)",border:`1px solid ${isDone?mod.color+"33":"rgba(255,255,255,0.06)"}`,transition:"all 0.18s"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:"0.85rem",fontWeight:500,color:isDone?"rgba(255,255,255,0.88)":"rgba(255,255,255,0.65)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:isMobile?"normal":"nowrap"}}>{l.title}</div>
...
function Tutor({chat,saveChat}){
  ...
  return(
    <div style={{maxWidth:760,margin:"0 auto",padding:isMobile?"16px 14px":"28px 20px",display:"flex",flexDirection:"column",height:"calc(100dvh - 60px)"}}>
